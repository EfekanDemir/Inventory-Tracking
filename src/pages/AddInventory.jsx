...
const AddInventory = () => {
  ...
  const loadDropdownData = async () => {
    try {
      ...
      // MAC adreslerini yeni tablodan yükle
      ...
      // Seri numaralarını yeni tablodan yükle
  ...
  const addHistoryRecord = async (equipmentId, operation = 'Güncelleme') => {
    try {
      // Değişiklikleri detaylı olarak hesapla
      const detailedChanges = []
      
      if (isEditMode && operation === 'Güncelleme') {
        const fieldsToTrack = [
          'mac_adresi', 'marka_id', 'model_id', 'lokasyon_id', 
          ...
          if (field.includes('tarihi')) {
            ...
            if (origDate !== currDate) {
              detailedChanges.push({
                ...
                old_display: getDisplayValue(field, originalValue),
                new_display: getDisplayValue(field, currentValue),
                change_type: 'update'
              })
            }
          } else {
      ...
      const historyData = {
        ekipman_id: equipmentId,
        islem_tipi: operation,
        eski_degerler: isEditMode ? {
          ...originalData,
          // Insan okunabilir değerler ekle
          _display_values: {
            marka: getDisplayValue('marka_id', originalData.marka_id),
            model: getDisplayValue('model_id', originalData.model_id),
            lokasyon: getDisplayValue('lokasyon_id', originalData.lokasyon_id),
            personel: getDisplayValue('atanan_personel_id', originalData.atanan_personel_id),
          }
        } : null,
        yeni_degerler: {
          ...formData,
          // Insan okunabilir değerler ekle
          _display_values: {
            marka: getDisplayValue('marka_id', formData.marka_id),
            model: getDisplayValue('model_id', formData.model_id),
            lokasyon: getDisplayValue('lokasyon_id', formData.lokasyon_id),
            personel: getDisplayValue('atanan_personel_id', formData.atanan_personel_id),
          }
        },
        degisiklik_sayisi: detailedChanges.length,
        yapan_personel_id: null, // Auth kaldırıldığı için null
      }

      // degisiklik_detaylari sütunu varsa ekle
      if (detailedChanges.length > 0) {
        historyData.degisiklik_detaylari = detailedChanges
      }
      ...
      console.error('Geçmiş kaydı ekleme hatası:', error)
      // Geçmiş kaydı hatası ana işlemi durdurmaz
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await addHistoryRecord(
        result.data.id, 
        isEditMode ? 'Güncelleme' : 'Yeni Kayıt'
      )
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth>
                  ...
                  >
                    ...
                    {existingMacAddresses.filter(mac => mac.kullanim_durumu === 'MUSAIT').length === 0 ? (
                      <MenuItem disabled>
                        <em>Sistem kurulumunda MAC adresi eklenmesi gerekli</em>
                      </MenuItem>
                    ) : (
                      existingMacAddresses.filter(mac => mac.kullanim_durumu === 'MUSAIT').map((mac) => (
                        <MenuItem key={mac.id} value={mac.mac_adresi}>
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth required>
                  ...
                  >
                    {existingSerialNumbers.filter(serial => serial.kullanim_durumu === 'MUSAIT').length === 0 ? (
                      <MenuItem disabled>
                        <em>Müsait seri numarası bulunamadı - Sistem kurulumu gerekli</em>
                      </MenuItem>
                    ) : (
                      existingSerialNumbers.filter(serial => serial.kullanim_durumu === 'MUSAIT').map((serial) => (
                        <MenuItem key={serial.id} value={serial.seri_no}>
