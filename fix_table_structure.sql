-- Tablo Yapısını Düzeltme Script'i
-- Eksik ID'leri ve Primary Key'leri ekleyelim
...
DECLARE
    r RECORD;
BEGIN
    RAISE NOTICE '=== MEVCUT TABLO YAPISI KONTROLÜ ===';
...
DO $$
BEGIN
    -- seri_numaralari tablosuna ID sütunu ekle (eğer yoksa)
    IF NOT EXISTS (
...
DO $$
BEGIN
    -- mac_adresleri tablosuna ID sütunu ekle (eğer yoksa)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
DO $$
BEGIN
    -- ekipman_gecmisi tablosuna ID sütunu ekle (eğer yoksa)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 4. Foreign Key constraint'leri düzelt
-- Cascade delete ekle
DO $$
BEGIN
...
BEGIN
    ...
    RAISE NOTICE 'ekipman_envanteri -> personel foreign key düzeltildi';
END $$;
